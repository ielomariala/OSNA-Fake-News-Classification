...
class DataTokenizerGen(tf.keras.utils.Sequence):
    ...
    def __init__(self, data, labels, batch_size, max_length, include_labels=True):
...
class DataLoader_Bert:
    ...
    def __preprocess(self, df, is_train=False):
        ...
        if is_train:
            ...
            labels = self.train_df["label"].apply(lambda x: 0 if x == "disagreed" else 1 if x == "agreed" else 2)
            y = tf.keras.utils.to_categorical(labels, num_classes=3)
        return self.tokenizer(df, y, batch_size, self.max_length, is_train)
    ...
    def get_test_set(self):
        ...
        self.test_gen = self.__preprocess(self.test_df)
        return self.test_gen